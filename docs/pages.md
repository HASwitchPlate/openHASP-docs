...
You can create the file wity your favourite text editor and upload it *(and other resource assets like fonts)* using the web interface [HASP Design](configuration/hasp.md) menu.
...
See [commands documentation](commands.md#jsonl) for the `jsonl` command. The payload of the command corresponds to what's exactly in one line of the `pages.jsonl` file above.
...
See [objects dodumentation](objects.md) for details.
...
Check out the [example](example-pagination.md) for how to implement a simple pagination bar.  
